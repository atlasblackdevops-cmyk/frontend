...
function resolveFieldKey(rawKey: string | undefined): string | undefined {
  ...
  if (key.includes('term') || key.includes('agree') || key.includes('condition')) return 'terms';
  return rawKey;
}
    ...
    if (Array.isArray(container)) {
      for (const entry of container as any[]) {
        ...
        if (typeof entry === 'object') {
          ...
          // { email: "msg" }
          const key =
            (entry.field as string) ?? (entry.property as string) ?? (entry.key as string);
...
export function AuthenticationForm({
  ...
  return (
    ...
    >
      ...
      <GoogleButton radius="xl" fullWidth mt="md">
        Sign with Google
      </GoogleButton>
      ...
      <Formik
        ...
        onSubmit={async (values, { setErrors, setSubmitting, setTouched, setStatus }) => {
          try {
            setStatus(undefined);
            const payload = {
            ...
            if (Object.keys(fieldErrors).length > 0) {
              ...
              const knownFields = new Set(['email', 'password', 'name', 'terms']);
              ...
              if (Object.keys(known).length > 0) {
                ...
                for (const key of Object.keys(known)) touchedFields[key] = true;
          ...
          <form onSubmit={handleSubmit}>
            ...
            <Stack gap="md">
              {type === 'register' && (
                <TextInput
                  ...
                  onBlur={handleBlur}
                  name="name"
                  error={(touched.name || submitCount > 0) && (errors.name as any)}
                  radius="md"
                  styles={{
                    input: {
                      borderColor: 'var(--mantine-color-gray-3)',
                      '&:focus': {
                        borderColor: 'var(--mantine-color-brandGreen-6)',
                      },
                    },
                  }}
                />
              ...
              <TextInput
                ...
                onBlur={handleBlur}
                name="email"
                error={(touched.email || submitCount > 0) && (errors.email as any)}
                radius="md"
                styles={{
                  input: {
                    borderColor: 'var(--mantine-color-gray-3)',
              ...
              <PasswordInput
                ...
                onBlur={handleBlur}
                name="password"
                error={(touched.password || submitCount > 0) && (errors.password as any)}
                radius="md"
                styles={{
                  input: {
                    borderColor: 'var(--mantine-color-gray-3)',
              ...
              {type === 'register' && (
                <Checkbox
                  ...
                  onChange={(event) => setFieldValue('terms', event.currentTarget.checked)}
                  error={(touched.terms || submitCount > 0) && (errors.terms as any)}
                  color="brandGreen"
                  styles={{
                    label: {
            ...
            >
              ...
              <Button
                ...
                styles={{
                  root: {
                    background: 'linear-gradient(135deg, var(--mantine-color-brandGreen-6) 0%, var(--mantine-color-brandOrange-6) 100%)',
                    border: 'none',
                    fontWeight: 600,
                    transition: 'transform 0.2s, box-shadow 0.2s',
                    '&:hover': {
                      transform: 'translateY(-2px)',
                      boxShadow: '0 4px 12px rgba(69, 228, 136, 0.4)',
                      background: 'linear-gradient(135deg, var(--mantine-color-brandGreen-7) 0%, var(--mantine-color-brandOrange-7) 100%)',
                    },
                  },
                }}
              >
