...
import type { AddIrrigationValues, AddIrrigationModalProps } from "../types";
import {
    VOLUME_UNIT_OPTIONS,
...
export default function AddIrrigationModal({
    opened,
    onClose,
    onSubmit,
    isSubmitting,
}: AddIrrigationModalProps) {
    ...
    const form = useForm<AddIrrigationValues>({
        ...
        validate: {
            ...
            irrigationMethod: (value) => (!value ? "Irrigation method is required" : null),
            waterVolume: (value) => (!value || value === "" ? "Water volume is required" : null),
            volumeUnit: (value) => (!value ? "Volume unit is required" : null),
            durationMinutes: (value) => {
                if (value === "" || value === undefined || value === null || (typeof value === 'number' && value <= 0)) {
    ...
    const submitDisabled =
        ...
        !form.values.irrigationMethod ||
        !form.values.waterVolume ||
        !form.values.volumeUnit ||
